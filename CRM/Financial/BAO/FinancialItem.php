...
class CRM_Financial_BAO_FinancialItem extends CRM_Financial_DAO_FinancialItem {
  ...
  public static function add($lineItem, $contribution, $taxTrxnID = FALSE, $trxnId = NULL) {
    ...
    if ($taxTrxnID) {
      ...
      if (($itemStatus == array_search('Unpaid', $financialItemStatus)) &&
        property_exists($contribution, 'revenue_recognition_date') &&
    ...
    if (empty($trxnId)) {
      ...
      if (empty($trxnId['id'])) {
        ...
        CRM_Contribute_BAO_Contribution::$_trxnIDs = NULL;
      }
    }
